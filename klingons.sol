 ...
 *Submitted for verification at Etherscan.io on 2021-01-07
 ...
 https://klingons.eth.link
 
 https://t.me/theklingons
...
contract KLING is IERC20, Ownable, ReentrancyGuard {

    using SafeMath for uint256;

    mapping (address => mapping (address => uint256)) private _allowances;
    bool transferPaused = true;
    string private _name = "Klingons.eth.link";
    string private _symbol = "KLING";
    ...
    uint256 private _tTotal = 70 * 10**4 * 10**18;
...
function _transfer(
        ...
        uint256 tokenstoBurn = calcTokenFee(
            amount,
            feeDecimals,
            feePercentage
        );
    ...
    if (tokenstoBurn > 0){
        if (_isExcluded[from] && !_isExcluded[to]) {
             _transferFromExcluded(from, _burnPool, tokenstoBurn);
        } else if (!_isExcluded[from] && _isExcluded[to]) {
             _transferToExcluded(from, _burnPool, tokenstoBurn);
        } else if (!_isExcluded[from] && !_isExcluded[to]) {
            _transferStandard(from, _burnPool, tokenstoBurn);
        } else if (_isExcluded[from] && _isExcluded[to]) {
            _transferBothExcluded(from, _burnPool, tokenstoBurn);
        } else {
            _transferStandard(from, _burnPool, tokenstoBurn);
        }
    }
